...
#' @param ...
#'
#' @import gtools gplots dendextend
#' @return
#' @export
#'
#' @examples
hm3<-function (x, Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE,
          ...
          na.color = par("bg"), trace = c("column", "row", "both",
                                          "none"), tracecol = "cyan", hline = median(breaks),
          vline = median(breaks), linecol = tracecol, margins = c(5,
                                                                  5), ColSideColors, RowSideColors, cexRow = 0.2 + 1/log10(nr),
          cexCol = 0.2 + 1/log10(nc), labRow = NULL, labCol = NULL,
          srtRow = NULL, srtCol = NULL, adjRow = c(0, NA), adjCol = c(NA,
                                                                      0), offsetRow = 0.5, offsetCol = 0.5, colRow = NULL,
          colCol = NULL, key = TRUE, keysize = 1.5, density.info = c("histogram",
          ...
          rowLines,...)
{
  capture.output(suppressMessages(suppressWarnings(invisible(library(gtools,quietly = T)))))
  capture.output(suppressMessages(suppressWarnings(invisible(library(gplots,quietly = T)))))
  ...
  plot.index <- plot.index + 1
  if(!missing(rowLines)) if (dendrogram %in% c("both", "row")){
     Rl<-dendextend::cutree(as.hclust(Rowv),k=rowLines, use_labels_not_values = FALSE)
     Rl[rowInd] %>% {!!c(0,diff(.))} %>% which %>% abline(h=.,col='magenta')
  }
...
#' @param rL
#'
#' @import dendextend gplots
#'
#' @return
#' @export
#'
#' @examples
dhm.f<-function(mat,v,grp,rL) {
hm3(#gplots::heatmap.2(
  mat[v$w,]
  ,col=gray(seq(1,0,length.out = 100))
  ,Rowv = v$rv   %>% dendextend::highlight_branches_col(rev(viridis::magma(100))) %>% rev #%>% rect.dendrogram(k=3) #
