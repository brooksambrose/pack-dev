...
#'
#' @examples
jpdf2imp<-function(jpdf,depth=5){
  require(data.table)
  require(stringr)
  require(pdftools)
  require(stringdist)
  ...
  # ocr
  browser()
  oc<-sapply(imp,function(x) x %>% unlist %>% nchar %>% sum)/sapply(imp,length)
  oc<-which(oc<200)
  if(!length(oc)){
    require(tesseract)
    for(i in oc){
  ...
  meta<-data.table(
    ...
    ,author=list(lapply(cp,function(x) {
      b<-grep('^Author',x)
      e<-grep('^Source',x)
      x[b:(e-1)] %>% paste(collapse=' ') %>% sub('^[^:]+: ','',.) %>% str_split(',|( and )') %>% unlist %>%
        sub('^ *','',.) %>% sub(' *$','',.) %>% sub(' +',' ',.) %>% `[`(.!='')
    }))
    ,source=list(lapply(cp,function(x) {
      b<-grep('^Source',x)
      e<-grep('^Published',x)
      x[b:(e-1)] %>% paste(collapse=' ') %>% sub('^[^:]+: ','',.) %>% str_split('[,)(]') %>% unlist %>%
        sub('^ *','',.) %>% sub(' *$','',.) %>% sub(' +',' ',.) %>% `[`(.!='')
    }))
    ,url=list(lapply(cp,function(x) {
      b<-grep('^Stable',x)
      x[b] %>% sub('^[^:]+: ','',.) %>% sub('^ *','',.) %>% sub(' *$','',.)
    }))
    ,accessed=list(lapply(cp,function(x) {
      b<-grep('^Accessed',x)
      x[b] %>% sub('^[^:]+: ','',.) %>% sub('^ *','',.) %>% sub(' *$','',.)
    }))
    )
  ...
  setcolorder(meta,c('doc','journal','year','title','author','source','url','accessed','doc'))
  # header detection, accurate solution is too slow
  pb <- progress_bar$new(format = "  [:bar] :elapsed :eta",total = i*depth*2, clear = FALSE, width= 60)
  clusth<-function(y) {
    y<-gsub(' +',' ',y)
    m<-1-stringdistmatrix(y,y,method = 'jw',p = .1)
    n<-graph_from_adjacency_matrix(m,'undirected',weighted=T)
    c<-cluster_spinglass(n) %>% communities
    r<-data.table(c=c)
    r[,s:=sapply(c,function(x) E(induced_subgraph(n,x))$weight%>% mean)]
    pb$tick()
    r
  }
  h<-lapply(imp,function(x) sapply(x,head,depth) %>% t %>% data.table %>% setnames(paste0('h',1:depth)))
  hc<-lapply(h,function(x) lapply(x,clusth))
  f<-lapply(imp,function(x) sapply(x,tail,depth) %>% t %>% data.table %>% setnames(paste0('f',1:depth)))
  fc<-lapply(f,function(x) lapply(x,clusth))
...
if(F){
  res<-microbenchmark(edge_betweenness=cluster_edge_betweenness(y),fast_greedy=cluster_fast_greedy(y),infomap=cluster_infomap(y),label_prop=cluster_label_prop(y),leading_eigen=cluster_leading_eigen(y),louvain=cluster_louvain(y),spinglass=cluster_spinglass(y),walktrap=cluster_walktrap(y))
