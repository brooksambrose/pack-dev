...
)
{
  library(igraph)
	...
	for(i in type) if(i%in%dir(mel2comps.dir)){
		...
		cos2kcc[[i]]$strict<-lapply(cos2kcc[[i]]$strict,function(x) key[list(x),gph])
		pdf(paste(out,paste('kcc2vis',i,'strict.pdf',sep='-'),sep=.Platform$file.sep))
		cols<-as.integer(sub('^k([0-9]+).*$','\\1',names(cos2kcc[[i]]$strict)))-2
		mark.col<-terrain.colors(max(cols))[cols]
		mark.border<-'white'
		plot(
			ret[[i]]$g
			# 			,mark.groups=tail(cos2kcc[[i]]$strict,1)
			# 			,mark.expand=5
			#			,mark.col=mark.col
		...
		trg<-list()
		for(j in length(cos2kcc[[i]]$orig):2) trg[[j]]<-graph.incidence(sapply(cos2kcc[[i]]$orig[[j]],function(x) sapply(cos2kcc[[i]]$orig[[j-1]],function(y) sum(x%in%y))),mode='in',weighted=T,directed=T)
		...
		pdf(paste(out,paste('kcc2vis',i,'sugiyama.pdf',sep='-'),sep=.Platform$file.sep))
		plot(trg
				 ,layout=trg$layout
				 ,vertex.label=NA
				 ,vertex.size=4
